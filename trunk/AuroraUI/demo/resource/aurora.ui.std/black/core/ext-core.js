...
(function() {
	var d = 0, r = navigator.userAgent.toLowerCase(), w = function(e) {
		return e.test(r)
	}, k = document.compatMode == "CSS1Compat", y = w(/opera/), g = w(/chrome/), s = w(/webkit/), v = !g
			&& w(/safari/), a = v && w(/version\/3/), z = v && w(/version\/4/), q = !y
			&& w(/msie/), o = q && w(/msie 7/), n = q && w(/msie 8/), p = q
			&& !o && !n, m = !s && w(/gecko/), b = m && w(/rv:1\.9/), t = q
			&& !k, x = w(/windows|win32/), j = w(/macintosh|mac os x/), h = w(/adobeair/), l = w(/linux/), c = /^https/i
	...
	Ext.apply(Ext, {
				...
				toArray : function() {
					return q ? function(e, C, A, B) {
						...
						return Array.prototype.slice.call(e, B || 0, A
										|| e.length)
					}
				}(),
				each : function(D, C, B) {
...
(function() {
	...
	(function() {
		...
		Ext.lib.Dom = {
			...
			getXY : function(l) {
				var k, r, t, w, m, n, v = 0, s = 0, u, j, o = (g.body || g.documentElement), q = [
						0, 0];
				l = Ext.getDom(l);
				if (l != o) {
					if (l.getBoundingClientRect) {
						...
						while (k) {
							...
							if (Ext.isGecko) {
								s += w = h(r.getStyle("borderTopWidth"), 10)
										|| 0;
								v += m = h(r.getStyle("borderLeftWidth"), 10)
										|| 0;
	...
	Ext.lib.Event = function() {
			...
			if (!E) {
				...
				Ext.each(r, function(N, O, M) {
							if (N && (J = H.getElementById(N.id))) {
								if (!N.checkReady || x || J.nextSibling
										|| (H && H.body)) {
									J = N.override ? (N.override === true
											? N.obj
											: N.override) : J;
									N.fn.call(J, N.obj);
									r[O] = null
		...
		var z = {
			...
			getRelatedTarget : function(I) {
				I = I.browserEvent || I;
				return this
						.resolveTextNode(I.relatedTarget
								|| (I.type == "mouseout"
										? I.toElement
										: I.type == "mouseover"
												? I.fromElement
	...
	Ext.lib.Ajax = function() {
		...
		function j(v, s, u, r) {
			...
			if (t) {
				...
				if (r && l.useDefaultHeader
						&& (!l.headers || !l.headers["Content-Type"])) {
	...
	(function() {
		var h = Ext.lib, k = /width|height|opacity|padding/i, g = /^((width|height)|(top|left))$/, d = /width|height|top$|bottom$|left$|right$/i, j = /\d+(em|%|en|ex|pt|in|cm|mm|pc)$/i, l = function(
				m) {
		...
		h.AnimBase.prototype = {
			...
			getAttr : function(m) {
				...
				return (!!(n[2]) || (o.getStyle("position") == "absolute" && !!(n[3])))
						? o.dom["offset" + n[0].charAt(0).toUpperCase()
								+ n[0].substr(1)]
						: 0
			},
			...
			init : function(n, r, q, m) {
				...
				var s = function() {
					var x = this;
					x.onTween.fire({
								duration : e() - x.startTime,
								curFrame : x.curFrame
							});
					var y = x.runAttrs;
					for (var w in y) {
						this.setAttr(w, x.doMethod(w, y[w].start, y[w].end),
								y[w].unit)
					}
					++p
				};
				var o = function() {
					var w = this, y = (e() - w.startTime) / 1000, x = {
						duration : y,
						frames : p,
						fps : p / y
					};
		...
		Ext.lib.AnimMgr = new function() {
			...
			var p = function(t) {
				var x = t.totalFrames, w = t.curFrame, v = t.duration, u = (w
						* v * 1000 / x), s = (e() - t.startTime), y = 0;
				if (s < v * 1000) {
		...
		(function() {
			...
			Ext.apply(h.Motion.prototype, {
				...
				getAttr : function(q) {
					...
					return n.test(q)
							? [r.call(s, "left"), r.call(s, "top")]
							: r.call(s, q)
				},
				doMethod : function(q, t, r) {
					var s = this;
					return n.test(q) ? h.Bezier.getPosition(s.runAttrs[q], s
									.method(s.curFrame, 0, 100, s.totalFrames)
									/ 100) : p.doMethod.call(s, q, t, r)
				},
				setRunAttr : function(x) {
					if (n.test(x)) {
						var z = this, s = this.el, C = this.attributes.points, v = C.control
								|| [], A = C.from, B = C.to, y = C.by, D = h.Dom, r, u, t, w, q;
	...
	(function() {
		...
		Ext.apply(g.Easing, {
			...
			easeBothStrong : function(m, l, o, n) {
				return ((m /= n / 2) < 1) ? o / 2 * m * m * m * m + l : -o / 2
						* ((m -= 2) * m * m * m - 2) + l
			},
			elasticIn : function(n, l, u, r, m, q) {
				...
				return -(m * h(2, 10 * (n -= 1)) * e((n * r - o) * (2 * k) / q))
						+ l
			},
			...
			elasticBoth : function(n, l, u, r, m, q) {
				...
				return n < 1 ? -0.5
						* (m * h(2, 10 * (n -= 1)) * e((n * r - o) * (2 * k)
								/ q)) + l : m * h(2, -10 * (n -= 1))
						* e((n * r - o) * (2 * k) / q) * 0.5 + u + l
			},
			...
			bounceOut : function(m, l, o, n) {
				if ((m /= n) < (1 / 2.75)) {
					...
					if (m < (2 / 2.75)) {
						return o * (7.5625 * (m -= (1.5 / 2.75)) * m + 0.75)
								+ l
					} else {
						if (m < (2.5 / 2.75)) {
							return o
			...
			bounceBoth : function(m, l, o, n) {
				return (m < n / 2) ? g.Easing.bounceIn(m * 2, 0, o, n) * 0.5
						+ l : g.Easing.bounceOut(m * 2 - n, 0, o, n) * 0.5 + o
						* 0.5 + l
			}
		})
	})();
	(function() {
		...
		var l = j.ColorAnim.superclass, k = /color$/i, g = /^transparent|rgba\(0, 0, 0, 0\)$/, n = /^rgb\(([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\)$/i, d = /^#?([0-9A-F]{2})([0-9A-F]{2})([0-9A-F]{2})$/i, e = /^#?([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})$/i, h = function(
				o) {
		...
		Ext.apply(j.ColorAnim.prototype, {
					getAttr : function(o) {
						var q = this, p = q.el, r;
						if (k.test(o)) {
	...
	(function() {
		...
		Ext.apply(d.Scroll.prototype, {
					doMethod : function(h, o, j) {
						var m, l = this, n = l.curFrame, k = l.totalFrames;
						if (h == e) {
							m = [l.method(n, o[0], j[0] - o[0], k),
									l.method(n, o[1], j[1] - o[1], k)]
						} else {
			...
			if (u.insertAdjacentHTML) {
				...
				if (y[z]) {
					...
					if (u.firstChild) {
						B[C](u[x]);
						D = B.createContextualFragment(A);
						z == n ? u.insertBefore(D, u.firstChild) : u
								.appendChild(D)
					} else {
						u.innerHTML = A
					}
	...
	function l() {
		if (!Ext.enableGarbageCollector) {
			...
			for (o in g.cache) {
				...
				q = p.dom;
				if (!q || !q.parentNode
						|| (!q.offsetParent && !j.getElementById(o))) {
					delete g.cache[o];
...
Ext.Element.addMethods(function() {
	var g = {}, u = /(-[a-z])/gi, b = {}, p = document.defaultView, r = Ext.isIE
			? "styleFloat"
			: "cssFloat", x = /alpha\(opacity=(.*)\)/i, k = /^\s+|\s+$/g, w = Ext.Element, d = "padding", c = "margin", v = "border", q = "-left", o = "-right", t = "-top", l = "-bottom", h = "-width", j = {
		l : v + q + h,
		r : v + o + h,
		t : v + t + h,
		b : v + l + h
	...
	return {
		...
		removeClass : function(z) {
			...
			if (A.dom.className) {
				Ext.each(z, function(B) {
							A.dom.className = A.dom.className
									.replace(
											b[B] = b[B]
													|| new RegExp(
															"(?:^|\\s+)"
		...
		hasClass : function(z) {
			return z
					&& (" " + this.dom.className + " ").indexOf(" " + z + " ") != -1
		},
		...
		getStyle : function() {
			return p && p.getComputedStyle ? function(C) {
				...
				C = s(C);
				return (z = B.style[C]) ? z : (A = p.getComputedStyle(B, ""))
						? A[C]
						: null
			} : function(D) {
				var B = this.dom, z, A;
		...
		setOpacity : function(A, z) {
			...
			if (!z || !D.anim) {
				if (Ext.isIE) {
					var C = A < 1 ? "alpha(opacity=" + A * 100 + ")" : "", E = B.filter
							.replace(x, "").replace(k, "");
					B.zoom = 1;
				...
				D.anim({
						...
						}, D.preanim(arguments, 1), null, 0.35, "easeIn")
			}
			return D
		},
		...
		setWidth : function(A, z) {
			var B = this;
			A = B.adjustWidth(A);
			!z || !B.anim ? B.dom.style.width = B.addUnits(A) : B.anim({
						width : {
							to : A
						}
					}, B.preanim(arguments, 1));
			return B
		},
		setHeight : function(z, A) {
			var B = this;
			z = B.adjustHeight(z);
			!A || !B.anim ? B.dom.style.height = B.addUnits(z) : B.anim({
						height : {
							to : z
						}
					}, B.preanim(arguments, 1));
		...
		clip : function() {
			...
			if (!y(dom, "isClipped")) {
				...
				y(dom, "originalClip,", {
							o : z.getStyle("overflow"),
							x : z.getStyle("overflow-x"),
							y : z.getStyle("overflow-y")
						});
...
(function() {
	...
	Ext.Element.addMethods({
				...
				setX : function(n, o) {
					return this.setXY([n, this.getY()], m.call(this, arguments,
									o, 1))
				},
				setY : function(o, n) {
					return this.setXY([this.getX(), o], m.call(this, arguments,
									n, 1))
				},
				setLeft : function(n) {
				...
				getLeft : function(n) {
					return !n ? this.getX() : parseInt(this.getStyle(b), 10)
							|| 0
				},
				getRight : function(n) {
					var o = this;
					return !n ? o.getX() + o.getWidth() : (o.getLeft(true) + o
							.getWidth())
							|| 0
				},
				getTop : function(n) {
					return !n ? this.getY() : parseInt(this.getStyle(d), 10)
							|| 0
				},
				getBottom : function(n) {
					var o = this;
					return !n ? o.getY() + o.getHeight() : (o.getTop(true) + o
							.getHeight())
							|| 0
				},
				...
				getPositioning : function() {
					...
					return {
						...
						left : n,
						right : n ? "" : this.getStyle(g),
						top : o,
						bottom : o ? "" : this.getStyle(j),
				...
				translatePoints : function(n, v) {
					v = isNaN(n[1]) ? v : n[1];
					n = isNaN(n[0]) ? n : n[0];
					var r = this, s = r.isStyle(h, e), u = r.getXY(), p = parseInt(
							r.getStyle(b), 10), q = parseInt(r.getStyle(d), 10);
...
(function() {
	...
	function a(s, n, p) {
		var q = Ext.id(), y = document, t = y.createElement("iframe"), r = Ext
				.getDom(s.form), x = [], w;
		t.id = t.name = q;
					...
					if (w.fireEvent(c, w, t)) {
						if (t.el) {
							if (!Ext.isEmpty(t.indicatorText)) {
								w.indicatorText = '<div class="loading-indicator">'
										+ t.indicatorText + "</div>"
							}
							if (w.indicatorText) {
								Ext.getDom(t.el).innerHTML = w.indicatorText
							}
							t.success = (Ext.isFunction(t.success)
									? t.success
									: function() {
						...
						var r = t.params, q = t.url || w.url, n, u = {
							success : b,
							failure : d,
							scope : w,
							argument : {
							...
							timeout : t.timeout || w.timeout
						}, s, x;
						if (Ext.isFunction(r)) {
							r = r.call(t.scope || g, t)
						}
						r = Ext.urlEncode(w.extraParams, typeof r == "object"
										? Ext.urlEncode(r)
										: r);
						if (Ext.isFunction(q)) {
						...
						n = t.method || w.method
								|| ((r || t.xmlData || t.jsonData) ? h : k);
						if (n === k
								&& (w.disableCaching && t.disableCaching !== false)
								|| t.disableCaching === true) {
							var v = t.disableCachingParam
									|| w.disableCachingParam;
							q += (q.indexOf("?") != -1 ? "&" : "?") + v + "="
									+ (new Date().getTime())
						}
						...
						return t.callback ? t.callback
								.apply(t.scope, [t, e, e]) : null
					}
				},
				isLoading : function(n) {
					return n
							? Ext.lib.Ajax.isCallInProgress(n)
							: !!this.transId
				},
				abort : function(n) {
					if (n || this.isLoading()) {
